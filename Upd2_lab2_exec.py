...
def main():
    ...
    while(loop_count > 0):
        ...
        move_arm(pub_command, loop_rate, home, 1.0, 1.0)

        rospy.loginfo("Sending goal 1 ...")

	angles = lab_invk(0.3,0.25,0.015,0)
	#anlge = lab_invk(0.3,0.25,0.055,0)
	#print("inverse",angles)

        move_arm(pub_command, loop_rate, Q[0][0][1], 1.0, 1.0)
        ...
        move_arm(pub_command, loop_rate, Q[0][0][0], 1.0, 1.0)
	if (digital_in_0 == 0):
	...
	angles = lab_invk(0.3,0.45,0.055,0)

        move_arm(pub_command, loop_rate, Q[2][0][1], 1.0, 1.0)
	#move_arm(pub_command, loop_rate, angles, 1.0, 1.0)

        rospy.loginfo("Sending goal 4 ...")
        move_arm(pub_command, loop_rate, Q[2][0][0], 1.0, 1.0)

	angles = lab_invk(0.3,0.45,0.018,0)
	#move_arm(pub_command, loop_rate, angles, 1.0, 1.0)
	print("qq",np.degrees([Q[2][0][1]]))
	print("q",np.degrees([Q[2][0][0]]))
